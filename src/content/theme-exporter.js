(function (global) {
  ...
  class LobnhoThemeExporter {
    ...
    static extractTheme(doc = document) {
      ...
      const token = (element) => { const style = computed(element); return { background: normalizeColor(style.backgroundColor), backgroundImage: style.backgroundImage && style.backgroundImage !== 'none' ? style.backgroundImage : null, color: normalizeColor(style.color), border: `${style.borderWidth} ${style.borderStyle} ${normalizeColor(style.borderColor)}`, borderRadius: normalizeRadius(style.borderRadius), boxShadow: normalizeShadow(style.boxShadow), height: style.height, minHeight: style.minHeight, minWidth: style.minWidth, display: style.display, gap: style.gap, padding: style.padding, paddingX: style.paddingLeft, paddingY: style.paddingTop, transition: style.transition, backdropFilter: style.backdropFilter !== 'none' ? style.backdropFilter : null }; };
      const empty = { meta: { site: '', url: '', extractedAt: new Date().toISOString(), generator: 'Lobnho Extension Theme Exporter v1.1.10' }, palette: {}, typography: {}, components: {}, effects: {}, tokens: {} };
      if (!doc.body) return empty;
      const MAX_ELEMENTS = 2500;
      const elements = Array.from(doc.querySelectorAll('*')).filter((element) => { const style = computed(element); return style && style.display !== 'none' && style.visibility !== 'hidden' && !['SCRIPT', 'STYLE', 'SVG'].includes(element.tagName); }).slice(0, MAX_ELEMENTS);
      ...
      const rootStyles = roots.map((element) => computed(element)).filter(Boolean);
      const rootColors = rootStyles.map((style) => normalizeColor(style.backgroundColor)).filter(Boolean);
      const darkGradient = rootStyles.map((style) => style.backgroundImage).find((image) => image && /gradient/i.test(image) && /(black|rgb\(\s*\d{1,2}\s*,\s*\d{1,2}\s*,\s*\d{1,2}|#[0-3][0-9a-f]{2,5})/i.test(image));
      const darkScheme = rootStyles.some((style) => style.colorScheme === 'dark') || doc.documentElement.matches(':root') && doc.documentElement.getAttribute('data-theme') === 'dark';
      const background = rootColors.find((color) => luminance(color) < 0.22) || rootColors.find((color) => luminance(color) < 0.45) || (darkGradient ? '#111827' : null) || (darkScheme ? '#111827' : null) || rootColors[0] || '#ffffff';
